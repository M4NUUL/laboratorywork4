...
async function runSmokeTest() {
  ...
  try {
    ...
    const admin = await request(baseUrl, '/api/auth/login', {
      method: 'POST',
      body: JSON.stringify({ login: 'admin', password: process.env.SEED_ADMIN_PASSWORD || 'Admin123!' }),
    });
    const operator = await request(baseUrl, '/api/auth/login', {
      method: 'POST',
      body: JSON.stringify({ login: 'operator', password: process.env.SEED_OPERATOR_PASSWORD || 'Operator123!' }),
    });
